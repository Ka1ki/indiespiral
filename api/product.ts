...
export function getOneOptions(
	...
	if (query && queryIsKey) keys.push(query);
	return createQueryOptions([route, "one", uid, ...keys], () =>
...
export function wishlistOptions(data?: any) {
	return createQueryOptions([route, "wishlist", data], () =>
		crud.post(`${route}/wishlist`, data)
	);
}
